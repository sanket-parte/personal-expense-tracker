...
You WILL create actionable task plans based on verified research findings. You WILL write three files for each task: plan checklist (`./.tracking/plans/`), implementation details (`./.tracking/details/`), and implementation prompt (`./.tracking/prompts/`).

**CRITICAL**: You MUST verify comprehensive research exists before any planning activity. You WILL use `.agent/workflows/task-researcher.md` (or perform comprehensive research manually if that workflow is unavailable) when research is missing or incomplete.

## Research Validation

**MANDATORY FIRST STEP**: You WILL verify comprehensive research exists by:

1. You WILL search for research files in `./.tracking/research/` using pattern `YYYYMMDD-task-description-research.md`
...
3. **If research missing/incomplete**: You WILL IMMEDIATELY use `.agent/workflows/task-researcher.md` (or equivalent research process)
4. **If research needs updates**: You WILL use `.agent/workflows/task-researcher.md` for refinement
...
- **WRITE**: You WILL create/edit files ONLY in `./.tracking/plans/`, `./.tracking/details/`, `./.tracking/prompts/`, and `./.tracking/research/`
...
**CRITICAL**: If you encounter invalid file references or broken line numbers, you WILL update the research file first using `.agent/workflows/task-researcher.md`, then update all dependent planning files.
...
**CRITICAL**: Research files MUST exist in `./.tracking/research/` before creating any planning files.

## Planning File Requirements

You WILL create exactly three files for each task:

### Plan File (`*-plan.instructions.md`) - stored in `./.tracking/plans/`

You WILL include:

- **Frontmatter**: `---\napplyTo: '.tracking/changes/YYYYMMDD-task-description-changes.md'\n---`
...
### Details File (`*-details.md`) - stored in `./.tracking/details/`
...
### Implementation Prompt File (`implement-*.md`) - stored in `./.tracking/prompts/`
...
```markdown
---
applyTo: ".tracking/changes/{{date}}-{{task_description}}-changes.md"
---
...
- .tracking/research/{{research_file_name}} - {{research_description}}
- Search specific terms: "{{org_repo}} {{search_terms}}" - {{implementation_patterns_description}}
...
- [ ] Task 1.1: {{specific_action_1_1}}
  ...
  - Details: .tracking/details/{{date}}-{{task_description}}-details.md (Lines {{line_start}}-{{line_end}})
...
**Source Research**: .tracking/research/{{date}}-{{task_description}}-research.md
...
- **Research References**:
  - .tracking/research/{{date}}-{{task_description}}-research.md (Lines {{research_line_start}}-{{research_line_end}}) - {{research_section_description}}
  - Search terms: "{{org_repo}} {{search_terms}}" - {{implementation_patterns_description}}
- **Dependencies**:
...
- **Research References**:
  - .tracking/research/{{date}}-{{task_description}}-research.md (Lines {{research_line_start}}-{{research_line_end}}) - {{research_section_description}}
  - Search terms: "{{org_repo}} {{search_terms}}" - {{patterns_description}}
- **Dependencies**:
  - Phase 1 completion
...
You WILL create `{{date}}-{{task_description}}-changes.md` in .tracking/changes/ if it does not exist.

### Step 2: Execute Implementation

You WILL follow .agent/antigravity/instructions.md
You WILL systematically implement .tracking/plans/{{date}}-{{task_description}}-plan.instructions.md task-by-task
...
1. You WILL provide a markdown style link and a summary of all changes from .tracking/changes/{{date}}-{{task_description}}-changes.md to the user:

   - You WILL keep the overall summary brief
   - You WILL add spacing around any lists
   - You MUST wrap any reference to a file in a markdown style link

2. You WILL provide markdown style links to .tracking/plans/{{date}}-{{task_description}}-plan.instructions.md, .tracking/details/{{date}}-{{task_description}}-details.md, and .tracking/research/{{date}}-{{task_description}}-research.md documents. You WILL recommend cleaning these files up as well.
3. **MANDATORY**: You WILL attempt to delete .tracking/prompts/{{implement_task_description}}.prompt.md
...
3. **If research missing/incomplete**: You WILL use `.agent/workflows/task-researcher.md` immediately
4. **If research needs updates**: You WILL use `.agent/workflows/task-researcher.md` for refinement
...
4. If content no longer exists, you WILL use `.agent/workflows/task-researcher.md` to update research
...
- **If research missing**: You WILL use `.agent/workflows/task-researcher.md` immediately
